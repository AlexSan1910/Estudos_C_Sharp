...
namespace Course{
    class Program{
        static void Main(string[] args){
            ...
            double areaX = Math.Sqrt(pX * (pX - x.A) * (pX - x.B) * (pX - xs.C)); // calculo da area do triangulo x

            //calculos Area trinagulo X
            double pY = (y.A + y.B + y.C) /  2.0;
            double areaY = Math.Sqrt(pY * (pY - y.A) * (pY - y.B) * (pY - y.C)); // calculo da area do triangulo y
